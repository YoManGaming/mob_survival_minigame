...
local function open_shop_page(page)
    ...
    for itemname, price in pairs(mob_survival.shop_items) do
        ...
        if minetest.registered_tools[itemname] then
            ...
            if split(itemname, ":")[1] == "rangedweapons" and not minetest.registered_craftitems[itemname].OnCollision then
...
function mob_survival.open_shop(player, page)
    ...
    if page == "misc" then
        formspec = formspec.."image_button[6.25,0.5;2,1.2;phone_button_yellow.png;goto_misc_btn;Misc]"
    else
        formspec = formspec.."image_button[6.25,0.5;2,1.2;phone_button_blue.png;goto_misc_btn;Misc]"
    end
		...
		if string.starts(field, "buy_") then
            ...
            if gold_player >= price then
                if minetest.registered_tools[name] then
                    ...
                    if split(name, ":")[1] == "rangedweapons" and not minetest.registered_craftitems[name].OnCollision then
