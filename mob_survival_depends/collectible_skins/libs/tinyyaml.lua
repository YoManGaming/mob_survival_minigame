...
local function checkdupekey(map, key)
  if map[key] ~= nil then
    -- gli aspetti sono le uniche cose a essere tabelle, ergo il controllo
    if type(map[key]) == "table" then
      assert(map[key] == nil, "[COLLECTIBLE SKINS] There are two or more skins with the same name (" .. key .. ") !")
    end
