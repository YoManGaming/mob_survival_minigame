...
function arena_lib.join_queue(mod, arena, p_name)
  ...
  -- notifico lɜ varɜ giocanti dellə nuovə giocante
  ...
  arena_lib.sound_play_all(arena, "arenalib_queue_join")
...
function arena_lib.remove_player_from_queue(p_name, has_disconnected)
  ...
  arena_lib.sound_play_all(arena, "arenalib_queue_leave")
...
function arena_lib.queue_to_arena(mod, arena)
  ...
  if arena.teams_enabled then
    ...
    for pl_name, pl_data in pairs(arena.players) do
      -- sometimes the assignment fails; investigating through this debug
      if not pl_data.teamID or not arena.teams[pl_data.teamID] then
        minetest.log("warning", "[ARENA_LIB] Teams assignment error! Arena = " .. arena.name .. ", player = " .. pl_name .. ", players = " .. dump(arena.players))
      end
