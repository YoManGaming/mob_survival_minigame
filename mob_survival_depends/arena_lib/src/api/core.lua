...
function arena_lib.change_teams_amount(sender, mod, arena_name, amount, in_editor)
  ...
  if curr_amount > amount then
    ...
    for i = 1, extra_teams do
      arena_lib.set_spawner(sender, mod, arena_name, amount + i, "deleteall", nil, nil, in_editor)
    end
  end
...
function arena_lib.toggle_teams_per_arena(sender, mod, arena_name, enable, in_editor)
  ...
  arena_lib.set_spawner(sender, mod, arena_name, nil, "deleteall", nil, nil, in_editor)
...
function arena_lib.set_custom_return_point(sender, mod, arena_name, pos, rot, in_editor)
  local id, arena = arena_lib.get_arena_by_name(mod, arena_name)
  ...
  if not pos then
    arena.custom_return_point = nil
  else
    if rot ~= nil and type(rot) ~= "table" then
      arena_lib.print_error(sender, S("Parameters don't seem right!"))
      return end

    arena.custom_return_point = {pos = pos, rotation = {x = rot.x, y = rot.y}}
  end
...
function load_settings(mod)
  ...
  if storage:get_string(mod .. ".SETTINGS") == "" then
    local default_settings = {
      return_point = { pos = {x = 0, y = 20, z = 0}, rotation = {x = 0, y = 0}},
    ...
    if not arena_lib.mods[mod].settings.return_point.pos then
      local pos = table.copy(arena_lib.mods[mod].settings.return_point)

      arena_lib.mods[mod].settings.return_point = {pos = pos, rotation = {x=0, y=0}}
...
function init_storage(mod, mod_ref)
  ...
  for i = 1, arena_lib.mods[mod].highest_arena_ID do
    ...
    if arena_str ~= "" then
      ...
      if arena.custom_return_point and not arena.custom_return_point.pos then
        local pos = table.copy(arena.custom_return_point)
        arena.custom_return_point = {pos = pos, rotation = {x=0, y=0}}
        to_update = true
      end
