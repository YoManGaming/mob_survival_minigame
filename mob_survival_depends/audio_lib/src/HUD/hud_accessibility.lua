...
function audio_lib.HUD_accessibility_stop(p_name, s_type, track_name)
  ...
  if s_type == "bgm" then
    ...
    panel:update({bg = BG .. "^[fill:1000x1000:0,0:#ff0000^[opacity:220", title_color = 0x000000})

    minetest.after(0.3, function()
      if not minetest.get_player_by_name(p_name) then return end
      panel:update({bg = BG .. "^[opacity:220", title_color = 0xffffff})

      if not panel:is_visible() or p_data.title ~= panel:get_info().title.text then return end
...
function hide_after_s(p_name, audio_name, duration)
  return minetest.after(math.max(duration, READING_DURATION), function()
