...
function mob_class:on_punch(hitter, tflp, tool_capabilities, dir)
	...
	local strength = false--ESA--mcl_potions.get_effect(hitter, "strength")
	local weakness = false--ESA--mcl_potions.get_effect(hitter, "weakness")
	...
	--ESA--mcl_enchanting not included
	-- if weapon then
		...
		if tool_capabilities.punch_attack_uses then
			...
			minetest.after(0, function(name)
				...
				if def.tool_capabilities and def.tool_capabilities.punch_attack_uses then
					...
					--ESA--tt.reload_itemstack_description(weapon) -- update tooltip
	...
	if damage >= 0 then
		...
		and tflp >= punch_interval then
			...
			if hitter and is_player then
				...
				kb = kb + 9 --ESA--mcl_enchanting not included-- * mcl_enchanting.get_enchantment(wielditem, "knockback")
