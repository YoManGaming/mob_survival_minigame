...
function arena_lib.join_arena(mod, p_name, arena_ID, as_spectator, is_forced)
  ...
  -- se entra come giocante
    ...
    for _, pl_name in ipairs(players) do
      ...
      local random_spawner = arena_lib.get_random_spawner(arena, teamID, true)
      local player = minetest.get_player_by_name(pl_name)

      player:set_pos(random_spawner.pos)
      player:set_look_horizontal(math.rad(random_spawner.rot.x))
      player:set_look_vertical(math.rad(random_spawner.rot.y))
      ...
      -- whom was spectating.
      -- Once fixed, use teleport_onto_spawner instead of set_pos and rot here above
...
function operations_before_entering_arena(mod_ref, mod, arena, arena_ID, p_name, as_spectator)
  ...
  -- sgancio eventuali giocanti figlɜ
  ...
  -- registro giocanti nella tabella apposita
...
function operations_before_playing_arena(mod_ref, arena, p_name)
  ...
  if table.indexof(mod_ref.disabled_damage_types, "fall") > 0 then
    ...
    p_armor_groups.fall_damage_add_percent = -100
    player:set_armor_groups(p_armor_groups)

  -- se invece era disabilitato dall'esterno ma c'è danno da caduta nel minigioco, imposto il flash
  elseif p_armor_groups.fall_damage_add_percent and p_armor_groups.fall_damage_add_percent == -100 then
    if not players_temp_storage[p_name].armor_groups then
      players_temp_storage[p_name].armor_groups = player:get_armor_groups()
    end

    p_armor_groups.fall_damage_add_percent = 0
    player:set_armor_groups(p_armor_groups)
  end
...
function operations_before_leaving_arena(mod_ref, arena, p_name, reason)
  ...
  local c_ret_point = arena.custom_return_point
  local clean_pos = c_ret_point and c_ret_point.pos or mod_ref.settings.return_point.pos
  local clean_rot = c_ret_point and c_ret_point.rotation or mod_ref.settings.return_point.rotation
  local noise_a = math.random(-1.5, 1.5)
  local noise_b = math.random(-1.5, 1.5)
  local noise_pos = {x = clean_pos.x + noise_a, y = clean_pos.y, z = clean_pos.z + noise_b}
  ...
  if clean_rot then
    if clean_rot.x then player:set_look_horizontal(math.rad(clean_rot.x + (noise_a * 10))) end
    if clean_rot.y then player:set_look_vertical(math.rad(clean_rot.y + (noise_b * 10))) end
  end
